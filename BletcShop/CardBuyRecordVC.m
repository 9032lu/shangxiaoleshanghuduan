...
#import "CardBuyRecordVC.h"
#import "CardBuyRecTableViewCell.h"
#import "UIImageView+WebCache.h"
@interface CardBuyRecordVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation CardBuyRecordVC
...
-(void)getdataRequest{
    NSString *url =[[NSString alloc]initWithFormat:@"%@MerchantType/member/getBuy",BASEURL];
    //http://101.201.100.191/cnconsum/App/MerchantType/member/getBuy
    ...
    [KKRequestDataService requestWithURL:url params:params httpMethod:@"POST" finishDidBlock:^(AFHTTPRequestOperation *operation, id result) {
        ...
        if (_index ==1) {
            ...
            self.dataSourse_A = [result[@"buy_list"] mutableCopy];
            
        }else{
            [self.dataSourse_A addObjectsFromArray:result[@"buy_list"]];
        }
       [head sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",HEADIMAGE,result[@"info"][@"headImage"]]] placeholderImage:[UIImage imageNamed:@"userHeader"]];
        ...
        NSString *costMoney=[NSString stringWithFormat:@"%@",[NSString getTheNoNullStr:result[@"buy_sum"] andRepalceStr:@"0"]];
        float total=[costMoney floatValue];
        totalMoney.text=[NSString stringWithFormat:@"办卡总额：%@元",[NSString stringWithFormat:@"%.2f",total]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.dataSourse_A.count!=0) {
        ...
        if ([self.navigationItem.title containsString:@"办卡"]) {
            ...
            cell.cardType.text = [NSString stringWithFormat:@"办卡卡种：%@", [NSString getTheNoNullStr:dic[@"card_type"] andRepalceStr:@"-"]];
            
            cell.costMoney.text = [NSString stringWithFormat:@"办卡金额：%@",[[NSString getTheNoNullStr:dic[@"sum"] andRepalceStr:@"-"] stringByAppendingString:@"元"]];
        }
