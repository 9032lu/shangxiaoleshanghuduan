...
#import "ChargeRecVC.h"
#import "CardBuyRecTableViewCell.h"
#import "UIImageView+WebCache.h"
@interface ChargeRecVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
-(void)getdataRequest{
    NSString *url =[[NSString alloc]initWithFormat:@"%@MerchantType/member/getRec",BASEURL];
    //http://101.201.100.191/cnconsum/App/MerchantType/member/getRec
    ...
    [KKRequestDataService requestWithURL:url params:params httpMethod:@"POST" finishDidBlock:^(AFHTTPRequestOperation *operation, id result) {
        ...
        if (_index ==1) {
            ...
            self.dataSourse_A = [result[@"rec_list"] mutableCopy];
            
        }else{
            [self.dataSourse_A addObjectsFromArray:result[@"rec_list"]];
        }
        ...
        totalMoney.text=[NSString stringWithFormat:@"充值总额：%@元",[NSString getTheNoNullStr:result[@"consum_sum"] andRepalceStr:@"0"]];
        
        NSString *costMoney=[NSString stringWithFormat:@"%@",[NSString getTheNoNullStr:result[@"rec_sum"] andRepalceStr:@"0"]];
        float total=[costMoney floatValue];
        totalMoney.text=[NSString stringWithFormat:@"充值总额：%@元",[NSString stringWithFormat:@"%.2f",total]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.dataSourse_A.count!=0) {
        ...
        if ([self.navigationItem.title containsString:@"充值"]) {
            
            cell.dateTime.text =[NSString stringWithFormat:@"%@",[NSString getTheNoNullStr:dic[@"datetime"] andRepalceStr:@"-"]] ;
            
            cell.cardType.text = [NSString stringWithFormat:@"充值卡种：%@", [NSString getTheNoNullStr:dic[@"card_type"] andRepalceStr:@"-"]];
            
            cell.costMoney.text = [NSString stringWithFormat:@"充值金额：%@",[[NSString getTheNoNullStr:dic[@"sum"] andRepalceStr:@"-"] stringByAppendingString:@"元"]];
        }
